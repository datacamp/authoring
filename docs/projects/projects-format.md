...
To be used to build a final live project, a Project notebook needs to follow a certain format. It is that format that is described here. If you are following [the Project creation process](projects-process.md) a step before creating the full Project notebook, is to create just the narrative part of the notebook. You can find the format of the narrative notebook [here](projects-narrative-format.md).

Overview of the format {#overview}
----------------------

A Jupyter notebook consists of a number of cells and the cell types used in a Project notebook are `code` cells and `markdown` cells. To further specify what each cell corresponds to in the final live Project cell *tags* are used. Here is how to display cell tags in Jupyter:

![](media/jupyter_tags.png)

The firsts cell in a Project notebook should be a `markdown` cell with the project title written as a one line H1 header. For example
...
1. `type:NotebookTask` : `markdown` - The title of the task. It should be a H2 header (`##`) and it should be numbered.
2. `@context` : `markdown` - The context for the task. Will be part of the students notebook.
...
7. `@tests` : `code` - Tests that checks if the student is doing the right thing in the task.
...
- The context is part of the "unfinished" student's notebook. When the student have finished the project it should read as a analysis/case study/blog post. Therefore the context should not be addressed to the student, but to a general audience, as if the project was already finished and published as a blog post.
  - Good: Now let's load in the the data and take a look at the first couple of rows.
...
This is the `markdown` cell that will be part of the instructions sidebar. It can contain any mardown. To include an image: Place the image in the `img/` folder, located in the same folder as the `project.ipynb` file, then link to it from the `markdown` cell like this: `![](img/my_favorite_horse.png)`.

**Style guide:**

- The `@instructions` should
  - Start with a bullet list with what's to be done in the task. Max 5 bullets.
...
The `@instructions` to the first task should include a "Good to know" section where you direct the student to resources that could be usefull throughout the Project. This could be external documentation, DataCamp courses and excercises, cheat sheets, Stack Overflow answers, etc. The "Good to know" headline should be written as a H3 header:
...
- Use `...` in python and `....` in R to indicate that code needs to be filled in.
  ...
  - For code where one or more lines are missing:
    ...
    # .... YOUR CODE FOR TASK 3 ....
    ```
- Structure the `@sample_code` so that there is a clear correspondance to the task instrucions. Rule of thumb: eEach bullet point in `@instructions` should correspond to a comment in the `@sample_code`.
...
This is the `code` cell that checks the Student's solution to the task. The main purpose of the `@tests` is to try to catch common errors and to give the student hints on how to resolve the errors. The purpose of the `@tests` is *not* to check every detail of the students solution. The idea is to just check *something* to give the student a sense of achievement when s/he completes a task.
